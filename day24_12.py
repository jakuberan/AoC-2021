...
def pre_calculate(inputs, memory, instr_nums=instr_nums, instr=instr):
    """
...
def check_zero_result(inputs, memory, instr_nums=instr_nums, instr=instr):
    """
...
while True:
    print(inputs)
    memory = pre_calculate(inputs.copy(), memory_zero.copy())
    output = check_zero_result(inputs.copy(), memory.copy())
    if 0 in output["z"]:
        if len(inputs) == 14:
